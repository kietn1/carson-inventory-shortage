...
# ============================================================
st.set_page_config(
    page_title="Carson Inventory Shortage Dashboard",
    page_icon="📦",
    layout="wide",
...
# ============================================================
# Carson file format mapping
# ============================================================
# File sample: CARSON 0602.xlsx / sheet Item_Activity_Report
# Header row: B:N
# B SKU | D Item Description | F Activity Date | G Trans. # | H Ref #
# I Qty in | K Qty out | M Balance | N Ctn Balance
CARSON_COLS = {
    "sku": 1,
    "description": 3,
    "activity_date": 5,
...
def first_qty_number(value) -> float:
    """Parse Qty like 400, ' / 0', '5,139 / 198'. Return the first number only."""
...
def find_header_row(raw: pd.DataFrame) -> int:
    for idx in range(len(raw)):
        ...
        has_balance = "balance" in row_values
        if has_sku and has_activity and has_ref and has_balance:
            return idx
    raise ValueError("Cannot find header row with SKU / Activity Date / Ref # / Balance.")
...
def extract_report_range(raw: pd.DataFrame):
    ...
    for r in range(min(15, len(raw))):
        row_text = " | ".join(clean_text(x) for x in raw.iloc[r].tolist())
        lowered = row_text.lower()
        if "item activity from" not in lowered:
            continue

        # Most Carson files store the range in one title cell.
        match = re.search(
        ...
        # Fallback if dates are split across cells.
        dates = []
...
def build_inventory_model(raw: pd.DataFrame) -> dict:
    ...
    for excel_row_num, row in rows.iterrows():
        sku_cell = clean_text(row.iloc[CARSON_COLS["sku"]] if len(row) > CARSON_COLS["sku"] else "")
        desc_cell = clean_text(row.iloc[CARSON_COLS["description"]] if len(row) > CARSON_COLS["description"] else "")
        activity_raw = row.iloc[CARSON_COLS["activity_date"]] if len(row) > CARSON_COLS["activity_date"] else None
        activity_text = clean_text(activity_raw)
        ref_text = clean_text(row.iloc[CARSON_COLS["ref_no"]] if len(row) > CARSON_COLS["ref_no"] else "")
        trans_no = clean_text(row.iloc[CARSON_COLS["trans_no"]] if len(row) > CARSON_COLS["trans_no"] else "")
        qty_in = first_qty_number(row.iloc[CARSON_COLS["qty_in"]] if len(row) > CARSON_COLS["qty_in"] else None)
        qty_out = first_qty_number(row.iloc[CARSON_COLS["qty_out"]] if len(row) > CARSON_COLS["qty_out"] else None)
        balance = first_qty_number(row.iloc[CARSON_COLS["balance"]] if len(row) > CARSON_COLS["balance"] else None)
        ctn_balance = first_qty_number(row.iloc[CARSON_COLS["ctn_balance"]] if len(row) > CARSON_COLS["ctn_balance"] else None)

        sku_lower = sku_cell.lower()
        activity_lower = activity_text.lower()
        ref_lower = ref_text.lower()

        # SKU section row: SKU appears in column B with item description in column D.
        if sku_cell and sku_lower not in {"sku", "totals:"}:
        ...
        # Carson format official total row is Column B = 'Totals:'.
        if sku_lower == "totals:":
        ...
        # Dated outbound transactions. Not Shipped still counts when it has a parsed date and Qty Out > 0.
    ...
    if sku_df.empty:
        raise ValueError("No SKU sections found in the Carson Item Activity Report.")
...
def to_excel_bytes(model: dict) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        display_table(model["beginning_balance_df"]).to_excel(writer, sheet_name="Beginning Balance Rows", index=False)
        ...
        for worksheet in writer.sheets.values():
            ...
            for col_cells in worksheet.columns:
                max_len = max(len(str(c.value)) if c.value is not None else 0 for c in col_cells[:1000])
                worksheet.column_dimensions[col_cells[0].column_letter].width = min(max(max_len + 2, 10), 38)
...
# Sidebar controls
# ============================================================
st.sidebar.title("📦 Carson Inventory")

uploaded = st.sidebar.file_uploader(
    "Drop Carson Item Activity Report here",
    type=["xlsx", "xls"],
    help="Expected format: SKU in column B, Activity Date in F, Ref # in H, Qty In in I, Qty Out in K, Balance in M.",
)
...
# Main app
# ============================================================
st.title("Carson Inventory Shortage")
st.caption("Built for the Carson Item Activity Report format.")

if uploaded is None:
    st.info("Upload the Carson Item Activity Report Excel file from the left sidebar to generate the dashboard.")
    st.stop()
...
try:
    ...
    st.error("File could not be processed. Please check if this is the Carson Item Activity Report format.")
    st.exception(exc)
    st.stop()
...
with st.expander("Export", expanded=False):
    st.download_button(
        ...
        file_name=f"carson_shortage_dashboard_export_{report_end.strftime('%Y%m%d')}.xlsx",
...
with audit_tab:
    ...
    st.caption("Verify recent outbound calculations and source rows from the Carson format.")
    ...
    for col, label, card_label in zip([r1, r2, r3], recent_labels, recent_card_labels):
        ...
        with col:
            metric_card(card_label, fmt_num(sku_df[label].sum()), f"{fmt_date(start)} - {fmt_date(end)} | {len(valid_dates)} working days")

    st.markdown("**Source Row Audit**")
    s1, s2, s3, s4 = st.columns(4)
    with s1:
        metric_card("Official Total Rows", fmt_num(len(model["official_total_df"])), "Column B = Totals:")
    with s2:
    ...
    with audit_group_col:
        audit_group = st.selectbox("Audit Category", ["Recent Outbound", "Official Source Rows", "Status Rows"])
...
with guide_tab:
    st.subheader("How this app reads the Carson file")
    st.markdown(
        """
        **Expected source format**
        - SKU section row: **Column B = SKU**, **Column D = Item Description**
        - Transaction rows: **Column F = Activity Date**, **G = Trans. #**, **H = Ref #**, **I = Qty In**, **K = Qty Out**, **M = Balance**
        - Official total row: **Column B = Totals:**
        - Ending balance row: **Column F = Ending Balance**
